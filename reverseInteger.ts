function calculateLuhn(n: number): number {
  ...
  for (let i = strN.length - 2; i >= 0; i -= 2) {
    ...
    if (doubleNum > 9) {
      doubleNum = Array.from(doubleNum.toString())
        .map(Number)
        .reduce((a, b) => a + b, 0);
    }
    checkSum += doubleNum;
...
function generateLuhn(): string {
  let cardNumber = [
    ...
    ...Array.from({ length: 14 }, () => Math.floor(Math.random() * 10)),
  ];
...
function checkCard(cardNumber: string): boolean {
  ...
  let cardNumberWithoutCheckDigit = cardNumber
    .slice(0, -1)
    .split("")
    .map(Number);
  let calculatedCheckDigit = calculateLuhn(
    parseInt(cardNumberWithoutCheckDigit.join(""))
  );
